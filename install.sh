...
case $DISTRO in
    arch)
        sudo pacman -S --needed python python-pip aria2 git
        ;;
    debian|ubuntu)
        sudo apt update
        sudo apt install -y python3 python3-pip python3-pyqt6 aria2 git
        ;;
    fedora)
        sudo dnf install -y python3 python3-pip python3-qt6 aria2 git
        ;;
    *)
        sudo apt update || true
        sudo apt install -y python3 python3-pip python3-pyqt6 aria2 git || true
        sudo dnf install -y python3 python3-pip python3-qt6 aria2 git || true
        sudo pacman -S --needed python python-pip aria2 git || true
        ;;
esac
...
echo ""
echo -e "${GREEN}🌶️  Enjoy FelfelDM!${NC}"#!/bin/bash
set -e
