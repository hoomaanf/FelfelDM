...
class ProxyDialog(QDialog):
    def __init__(
        ...
        # Info label
        info_label = QLabel(
            "💡 Proxy will be used for all downloads (HTTP, HTTPS, FTP)"
        )
    ...
    def _test_proxy(self):
        ...
        if not config.is_valid():
            self.status_label.setText(
                "❌ Invalid proxy configuration (check host and port)"
            )
        ...
        try:
            ...
            response = requests.get(
                "https://www.google.com", proxies=proxies, timeout=10, verify=True
            )
            ...
            self.status_label.setText(
                "❌ SSL error - try using HTTP proxy or disable SSL verification"
            )
            self.status_label.setStyleSheet("color: #e74c3c; font-size: 11px;")
