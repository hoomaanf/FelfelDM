...
class Aria2Handler(QObject):
    ...
    def resume(self, gid: str):
        ...
        try:
            ...
            self.operation_result.emit("resume", {"gid": gid, "success": False, "error": str(e)})
    ...
    def pause(self, gid: str):
        try:
            ...
            self.operation_result.emit("pause", {"gid": gid, "success": False, "error": str(e)})
    ...
    def remove(self, gid: str):
        try:
            ...
            self.operation_result.emit("remove", {"gid": gid, "success": False, "error": str(e)})
    ...
    def add_url(self, url: str, options: dict):
        try:
            ...
            self.operation_result.emit("add_url", {"url": url, "gid": new_gid, "success": True})
            print(f"➕ [Handler] Added URL: {url} -> {new_gid}")
        except Exception as e:
            self.operation_result.emit("add_url", {"url": url, "gid": None, "success": False, "error": str(e)})
    ...
    def re_add(self, old_gid: str):
        ...
        try:
            ...
            if not url or not save_path:
                ...
                self.operation_result.emit("re_add", {"old": old_gid, "new": None, "success": False})
                return
            ...
            if new_gid:
                ...
                self.operation_result.emit("re_add", {"old": old_gid, "new": new_gid, "success": True})
            else:
                self.operation_result.emit("re_add", {"old": old_gid, "new": None, "success": False})
        except Exception as e:
            print(f"⚠️ [Handler] Re-add failed: {e}")
            self.operation_result.emit("re_add", {"old": old_gid, "new": None, "success": False, "error": str(e)})
    ...
    def set_speed_limit(self, gid: str, speed_kb: int):
        try:
            ...
            self.operation_result.emit("set_speed_limit", {"gid": gid, "speed": speed_kb, "success": True})
            print(f"⚡ [Handler] Speed limit set for {gid}: {limit}")
        except Exception as e:
            self.operation_result.emit("set_speed_limit", {"gid": gid, "speed": speed_kb, "success": False, "error": str(e)})
    ...
    def save_session(self):
        try:
            ...
            self.operation_result.emit("save_session", {"success": False, "error": str(e)})
